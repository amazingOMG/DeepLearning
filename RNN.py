...
from keras.layers import SimpleRNN , Dense , Dropout # 全連接層、Dropout層
...
def preprocessing():  # 前置處理
    ...
    image_Train = image_Train.reshape( -1 , 28 , 28 ).astype( 'float32' )  # 將二維圖片資料( 28 * 28 = 784 )轉換為三維資料
    image_Test = image_Test.reshape( -1 , 28 , 28 ).astype( 'float32' )
...
def layers( classification ):  # 設置輸入層、隱藏層、輸出層
    model.add( SimpleRNN( input_shape = (  28 , 28 ) , units = 50 , activation = 'relu' ) ) # 輸出層與隱藏層  # input_shape：影像尺寸 。

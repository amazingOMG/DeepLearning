...
from keras.models import Sequential  # 建立訓練模型
from keras.utils import np_utils  # 後續將label標籤轉換成One-hot encoding需要
from keras.layers import Dense , Dropout # 全連接層、Dropout層
import numpy as np # 矩陣相關運算需要此套件
import matplotlib.pyplot as plt  # 將資料視覺化，可用圖表呈現結果
import pandas as pd # 混淆矩陣需要此套件

def preprocessing( data_Train , data_Test ):  # 前置處理
    global image_Train , image_Test

    image_Train = image_Train.reshape( data_Train , 1 * 784 ).astype( 'float32' )  # 將二維圖片資料( 28 * 28 = 784 )攤平為一維資料( 1 * 784 )
    image_Test = image_Test.reshape( data_Test , 1 * 784 ).astype( 'float32' )
...
def Label_Image( label , image , predict , index ):
    plt.figure(  figsize = ( 12 , 14 )  ) # 設置圖形大小為 1200 * 1400

    for loop in range( 0 , 10 ):
        ax = plt.subplot( 5 , 5 , 1 + loop )
        image = image.reshape( len( image ) , 28 , 28 ).astype( 'float32' )  # 將一維圖片資料( 1 * 784 )轉換為二維資料( 28 * 28 )
        ax.imshow( image[ index[loop] ] , cmap = 'binary' )  # 定義測試集的輸出影像
...
nb_classes = 10  # 數字圖片為0~9，共有10種類別
preprocessing( len( image_Train ) , len( image_Test ) ) # 進行前置處理
...
# compile：編譯模型    loss：損失函數(crossentropy為交叉熵)     optimizer：優化器( adam：基於隨機梯度下降法（SGD）的優化演算法)  metrics：模型評估方式(accutacy：以準確度為主)
...
print(  '測試集的準確度 = {:.2f}%'.format( accuracy * 100 )  )

################################################### 以上為評估模型 #####################################################################

prediction = np.argmax( model.predict( image_Test ) , axis = -1 ) # 預測測試集內的數字影像資料
print( '\n\n測試集內 的 數字影像資料 預測結果為：' + str( prediction ) + '\n\n' ) # 輸出預測結果
